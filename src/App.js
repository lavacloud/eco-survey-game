...
const EcoSurvey = () => {
  ...
  // Function to submit data to Supabase using fetch API
  const submitToSupabase = async () => {
    ...
    try {
      ...
      questions.forEach((question, index) => {
        // Use the question text as the key and the answer as the value
  ...
  useEffect(() => {
    // Only attempt to submit if we've reached the end of the survey
        ...
        <div className="flex justify-center gap-8">
          <button
            onClick={() => {
              ...
              // Add a small delay to show the selection before moving on
              setTimeout(() => {
                handleAnswer("No");
